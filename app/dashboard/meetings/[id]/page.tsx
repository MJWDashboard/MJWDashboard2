...
export default async function MeetingModePage({ params }: { params: { id: string } }) {
  ...
  const [{ data: notes }, { data: tenants }, { data: buildings }] = await Promise.all([
    supabase
      .from("meeting_notes")
      .select("id, note, tenant_id, converted_to_action_id, created_at, tenants(trading_name)")
    ...
    <div>
      ...
      {meeting.agenda && (
        <div className="card mb-6">
          ...
          <p className="whitespace-pre-wrap text-sm text-charcoal-300">{meeting.agenda}</p>
        </div>
      )}
